...
export async function GET(req: Request) {
  ...
  let q = supabase
    ...
    .select('id, codigo, created_at, monto, status, sustento_url, beneficiario_nombre, descripcion, tipo, proyecto:proyectos(nombre)')
  ...
  ws.columns = [
    { width: 12 }, { width: 12 }, { width: 30 }, { width: 34 },
  ...
  head.values = ['Fecha', 'Código', 'Proyecto', 'Detalle', 'Monto', 'Estado', 'Sustento'];
  head.eachCell((c) => {
  ...
  filas.forEach((f) => {
    ...
    if (f.sustento_url) {
      row.getCell(7).value = { text: 'Ver sustento', hyperlink: f.sustento_url };
